...
#-------------------------------------------------------------------------------------------
# DETECCIÓN DE DISPONIBILIDAD DE TOTALSEGMENTATOR
...
def _try_python_api(input_nii: nib.Nifti1Image) -> dict:
    ...
    output_nii = _totalseg_api(
        ...
        fast=True, # Rápida y menos precisa, suficiente para localización
        ml=True,   # Multilabel: devuelve un volumen único donde cada estructura tiene su propio valor entero
        verbose=False,
    )
    ...
    # Si no se encuentra nada por 17/18 pero sí hay vóxeles no-cero en la máscara,
    # asume que ha ocurrido la reetiquetación y toma los dos valores más bajos como 
    # riñones izquierdo/derecho.
...
#-------------------------------------------------------------------------------------------
# INTENTO 2: CLI (Command Line Interface)
...
def _fallback_localize(volume_hu: np.ndarray, spacing: tuple) -> dict:
    ...
    tissue = ndimage.binary_opening(tissue,  iterations=2) # Apertura binaria para eliminar ruido
    tissue = ndimage.binary_closing(tissue,  iterations=3)  # Cierre binario para rellenar huecos

    #Componentes conectados. De todas las regiones detectadas, asume que los dos riñones son las dos 
    ...
    # Separa las regiones calculando el centroide X de toda la máscara combinada y dividiendo el volumen en dos mitades.
...
#-------------------------------------------------------------------------------------------
# FUNCIÓN PÚBLICA con la cadena de fallbacks
