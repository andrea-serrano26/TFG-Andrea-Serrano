...
//--------------------------------------------------------------------------------------------------------------------------------
// CARGA Y RENDERIZADO DE MALLA STL
...
function TumorMesh({ stlUrl, visible }: { stlUrl: string | null; visible: boolean }) {
  ...
  useEffect(() => {
    ...
    loader.load(stlUrl, 
      (geo) => {
        geo.computeBoundingBox(); // Se asegura de que la malla esté centrada en el origen
        const center = new THREE.Vector3();
        geo.boundingBox!.getCenter(center);
        geo.translate(-center.x, -center.y, -center.z);
        geo.computeVertexNormals(); // Mejora la iluminación de la malla
        setGeometry(geo);
      },
      undefined,
      () => setError(true)
  ...
  // Mientras se carga o hay error, se muestra un placeholder (esfera translúcida)
  ...
  // Renderiza la malla STL con material estándar
  // side={THREE.DoubleSide} asegura que se vea desde ambos lados
  return (
...
// Gizmo de orientación con ejes y etiquetas (S/I, A/P, L/R)
// Seis direcciones que representan las orientaciones anatómicas
const GIZMO_AXES = [
...
function GizmoOverlay({ q }: { q: THREE.Quaternion }) {
  ...
  // Mini renderizado SVG del gizmo de orientación, con líneas y etiquetas para cada eje
  return (
    <div style={{
...
//--------------------------------------------------------------------------------------------------------------------------------
// COMPONENTE PÚBLICO Y LÓGICA DE CARGA DEL STL
...
return (
    <div className="h-full flex flex-col bg-[#b3caec] overflow-hidden">
      ...
      <div className="flex-1 relative overflow-hidden bg-[#b3caec]">
        ...
        >
          {/* Se pasa setCamQ como onRotation para que el gizmo funcione */}
