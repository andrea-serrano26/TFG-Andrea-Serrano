...
N_CLASSES      = 3
ROI_MARGIN_MM  = 80   # 30mm era insuficiente: generaba 43% de padding en el patch

# ── Configuración de inferencia ───────────────────────────────────────────────
#
#  Con MPS (Apple Silicon GPU) el tiempo por patch baja de ~16s a ~1-2s, lo que
#  permite usar overlap=0.5 (igual que entrenamiento) en ~1-2 min.
...
USE_TTA     = False  # Desactivado en Apple Silicon: np.flip sobre arrays grandes causa segfault
# ─────────────────────────────────────────────────────────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
# ARQUITECTURA KiTSUNet (idéntica a la de entrenamiento)
# ══════════════════════════════════════════════════════════════════════════════
...
def _get_device() -> torch.device:
    ...
    # MPS: acelerador GPU de Apple Silicon.
    # En PyTorch ≥2.0 Conv3D está soportado nativamente en MPS.
    # Las versiones anteriores hacían fallback a CPU (más lento que CPU directo).
    # Si hay problemas, comentar esta línea para forzar CPU.
...
def run_segmentation(volume_hu: np.ndarray, spacing: tuple,
    ...
    #
    #   PROBLEMA ANTERIOR: procesar cada riñón con margen=30mm generaba un ROI
    #   de solo ~109 voxels en X, pero PATCH_SIZE requiere 192 → el 43% del
    #   patch era padding artificial (tejido reflejado). Con solo 4 patches
    #   totales el tumor casi no se segmentaba.
    #
    #   SOLUCIÓN: fusionar ambos riñones y usar margen=80mm. El ROI combinado
    #   tiene ~393 voxels en X → cero padding, 64 patches, contexto anatómico
    #   completo (ambos riñones, hígado, columna), igual que el entrenamiento.
    #
    ...
    tumor_iso      = (mask_iso == 2).astype(np.float32)
