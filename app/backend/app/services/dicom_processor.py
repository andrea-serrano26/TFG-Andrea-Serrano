...
def apply_windowing(arr: np.ndarray, wc: float = 40.0, ww: float = 400.0) -> np.ndarray:
    """Convierte valores HU a uint16 para visualización usando window center (wc) y window width (ww).
    El resultado se normaliza a 0-65535 para preservar el rango completo en uint16.
    """
    lo = wc - ww / 2.0
    hi = wc + ww / 2.0
...
#--------------------------------------------------------------------------------------------------------------------------------
# HELPERS PARA MANEJO DE CACHE Y ACTUALIZACIÓN DE MÁSCARA

#def get_file_type(session_id):
#    return memory_cache.get("file_type", "dicom")

def update_mask_from_edit(session_id, new_mask):
...
def load_nifti_mask(nifti_path: str, session_id: str) -> np.ndarray:
    """
    # 1. Cargar con nibabel
    # 2. Convertir coordenadas RAS a LPS (alinear con DICOM)
    # 3. Transponer y voltear para igual orientación
    # 4. Resamplear si es diferente tamaño al volumen CT
    # 5. Limpiar caché de slices para forzar re-renderizado con la nueva máscara
    # 6. Devolver máscara binaria uint8
"""
    ...
    data = nii_ras.get_fdata()
    data = np.transpose(data, (2, 1, 0))
    data = data[:, ::-1, :]
    mask = (data > 0.5).astype(np.uint8)

    if memory_cache["session_id"] == session_id and memory_cache["volume"] is not None:
        vol_shape = memory_cache["volume"].shape
        if mask.shape != vol_shape:
            factors = tuple(v / m for v, m in zip(vol_shape, mask.shape))
            mask = nd_zoom(mask, factors, order=0).astype(np.uint8)
...
def numpy_to_base64(
    ...
    if is_mask:
        # Máscara: RGBA semitransparente
        rgba = np.zeros((rows, cols, 4), dtype=np.uint8)
        rgba[arr > 0] = [255, 207, 38, 180]
        img = Image.fromarray(rgba, "RGBA")
        if (out_h, out_w) != (rows, cols):
            # Usar NEAREST para máscara (preserva bordes nítidos)
            img = img.resize((out_w, out_h), Image.NEAREST)
        img.save(buf, format="PNG", compress_level=1)
    
    else:
        # CT: Aplicar windowing o preservar HU 
