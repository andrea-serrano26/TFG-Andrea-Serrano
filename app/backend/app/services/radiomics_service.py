...
def extract_radiomics(mask_zyx, volume_zyx, spacing_zyx) -> dict:
    ...
    # Durante el entrenamiento los NRRDs se cargaron sin spacing → PyRadiomics asumió (1,1,1).
...
# Creo que se puede eliminar esta función, no se usó en el modelo final y no aporta mucho valor clínico en este caso.
def surface_area_mm2(mask: np.ndarray, spacing_zyx: tuple) -> float:
    """
    Calcula el área de superficie de la máscara usando Marching Cubes.
    Devuelve el área en mm².
    """
    try:
        from skimage import measure
        verts, faces, _, _ = measure.marching_cubes(mask, level=0.5, spacing=spacing_zyx)
        # Área = suma de áreas de todos los triángulos de la malla
        v0 = verts[faces[:, 0]]
        v1 = verts[faces[:, 1]]
        v2 = verts[faces[:, 2]]
        ...
        print(f"  No se pudo calcular área de superficie: {e}")
        return 0.0
...
def analyze_tumor(mask: np.ndarray, volume_hu: np.ndarray, spacing: tuple) -> dict:
    ...
    try:
        ...
        prob_ccr = float(model.predict_proba(X.to_numpy())[0][1]) * 100
        prob_ccr = round(min(max(prob_ccr, 1.0), 99.0), 1)
        source = "model"
    ...
    is_ccr = prob_ccr >= 50.0

    prob_ccr = float(model.predict_proba(X.to_numpy())[0][1]) * 100
    print(f"prob_ccr raw: {prob_ccr:.2f}%")
